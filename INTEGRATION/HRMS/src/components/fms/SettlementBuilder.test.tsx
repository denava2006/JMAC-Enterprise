...
vi.mock('@/hooks/useTreasury', () => ({
  ...
  useUnsettledCollections: (
    ...
    // No branch, nothing offered -- the server behaves the same way, and a
    // mock that answered across branches would hide the rule under test.
    const rows =
      opts.branchId === 'b1' ? [cavite] : opts.branchId === 'b2' ? [mainOffice] : []
...
describe('what the Branch control says in provider mode', () => {
  ...
  // A settlement is one branch's money. Nothing is offered until the
  // Accountant says whose: an earlier version defaulted a provider payout to
  // every branch, so collections appeared before anyone had chosen.
  it('opens with no branch chosen', async () => {
  ...
  it('shows nothing, and no way to select it, until a branch is chosen', async () => {
    await inProviderMode()
    expect(
      screen.getByText(/Choose a branch to see its unsettled provider collections/i)
    ).toBeTruthy()
  ...
  // Changing the branch must clear what was ticked under the old one: those
  // sales are not on this list, and the server would refuse them.
  it('clears the selected sales when the branch changes', async () => {
    await inProviderMode()
    await pick('Branch', 'Cavite Branch')
  ...
  it('never offers All branches, in either mode', async () => {
    // Branch cash first, then provider: the option exists nowhere, because a
    // settlement that spans branches cannot be reconciled by any one of them.
    render(<SettlementBuilder open onOpenChange={() => {}} />)
